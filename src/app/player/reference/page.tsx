...
import { getPlayer, getRecentRecords } from "@/lib/data";
...
export default async function ReferencePage() {
  ...
  if (!player?.profile) return <p className="text-center text-ink-3 mt-8">データを読み込めませんでした</p>;

  const position = player.profile.position as Position;
  const positionDetail = getDetailedPositionData(position);
